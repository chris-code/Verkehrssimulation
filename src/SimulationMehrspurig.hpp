...
class SimulationMehrspurig {
	public:
		SimulationMehrspurig( double minDallyFactor, double maxDallyFactor, double lambdaRiskFactor,
		                      double maxSpeedMean,
		                      double maxSpeedStd )
			: road( 0, 0 ),
		...
		void addCars() {
			for( auto l = road.getLaneCount() - 1; l >= 0; --l ) {
				if( road.getVehicle( 0, l ) == nullptr && road.getDensity() < carDensity ) {
					Vehicle* v = new Vehicle( randomEngine, dallyFactorDistribution, riskFactorDistribution,
		...
		void checkDistances() {
			for( auto s = 0; s < road.getStreetLength(); ++s ) { // iterate over all segments
				for( auto l = 0; l < road.getLaneCount(); ++l ) { // iterate over all lanes
					Vehicle* v = road.getVehicle( s, l );
					if( v != nullptr ) {
						for( long lane = l; lane >= 0;
						        --lane ) { // iterate over lanes that are not on the right of the considered vehicle
